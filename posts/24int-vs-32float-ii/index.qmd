...
Kesalahan algoritma? Penulis kemudian melakuan uji coba terhadap algoritma dengan cara mengambil sebuah *file* WAV 16-bit yang sudah ada, kemudian melakukan konversi *bit depth* dari *file* tersebut ke `float32` dan `int24`. Karena data sinyal telah terkuantisasi dari rekaman 16-bit, level yang digunakan oleh konversi `float32` dan `int24` dari *file* tersebut seharusnya memberikan nilai yang sama dan berada di sekitar $2^{16}=65536$. Dan benar, dari *file* didapatkan keluaran berikut:

```{code-line-numbers="false"}
'pn35a-f32.wav' (format: 3, channels: 2, sampling rate: 44100 Hz, bit depth: 32 bits) has 65536 uniques samples
'pn35a-i24.wav' (format: 1, channels: 2, sampling rate: 44100 Hz, bit depth: 24 bits) has 65536 uniques samples
```
